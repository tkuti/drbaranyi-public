...
import { getACertainFullWeek, getWeekNumber } from '../../../DateHelperFunctions'
...
function AdminCalendar() {
    ...
    const isInactive = (date) => {
        return ((actualWeek.indexOf(date) === 5 || actualWeek.indexOf(date) === 6)
            && specialDays.findIndex(specDay =>
                specDay.day.slice(0, 10) === date && specDay.type === "active") < 0
        )
            || specialDays.findIndex(specDay =>
                specDay.day.slice(0, 10) === date && specDay.type === "inactive") >= 0
    }

    const isSpecialDay = (date) => {
        const specialDay = specialDays.find(specDay => specDay.day.slice(0, 10) === date)
    ...
    return (
        <div className="card-box admin-container" >
                    ...
                    <Col sm={12} lg={3} >
                        ...
                        }
                        {
                            ...
                            <DisplayReservation
                                ...
                                selectedTime={selectedTime}
                                selectedEvent={appointments.find(app => app.day === selectedTime.day && app.time === selectedTime.time)}
                                setResponse={setResponse}
                            ></DisplayReservation>
                        }
                    </Col>
